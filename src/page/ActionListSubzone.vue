...
<template>
  <div class="table-wrapper">
    ...
    <table-item v-for="item in items" v-bind:data="item" v-bind:key="item.text" v-on:click.native="goDealer(item.dealerId, item.dealerName)">
      <a slot="title"><span class='item-left'>{{item.dealerName}}({{item.sum}})</span> </a>
      <a slot="state">
...
<script>
  ...
  export default {
    ...
    beforeRouteLeave (to, from, next) {
      if (to.name === 'DealerAction') {
        this.$localStorage.set('alszPage', this.currPage)
      }
//      console.log('组件路由勾子：beforeRouteLeave: ' + this.currPage)
      next()
    },
    created: function () {
      this.currPage = this.$localStorage.get('alszPage')
    ...
    methods: {
      fetchData: function () {
        ...
        HTTP.get('/actiondetail/bysubzone?subzone=' + this.subzoneId + '&page=' + this.currPage + '&size=' + this.pageSize)
...
//            console.log(response.data)
      ...
      fetchDataByPage: function (size) {
        ...
        HTTP.get('/actiondetail/bysubzone?subzone=' + this.subzoneId + '&page=1&size=' + size)
          .then(function (response) {
//            console.log(response.data)
            this.items = response.data.data
      ...
      goDealer: function (id, name) {
        router.push({name: 'DealerAction', params: {dealerId: id, dealerName: name}})
      },
      loadBottom: function () {
        this.currPage ++
        HTTP.get('/actiondetail/bysubzone?subzone=' + this.subzoneId + '&page=' + this.currPage + '&size=' + this.pageSize)
