...
<template>
  <div class='page-wrapper'>
    ...
    <table-item v-on:click.native="goReportList">
      <a slot='title'><span class='item-left'>走访报告－{{zoneName}}大区</span></a>
      <a slot='state'>
        <Icon name='angle-right' scale='1.5'></Icon>
      </a>
    </table-item>

    <table-item v-for="item in items" :data="item" :key="item.id" v-on:click.native="goSubzone(item.id, item.name)">
  ...
  export default {
    ...
    methods: {
      fetchData: function () {
        ...
        HTTP.get('/subzones/byzone?zone=' + this.zoneId)
...
//            console.log(response.data)
      ...
      goReportList: function () {
        router.push({name: 'Reportlists', params: {currType: '2', zoneId: this.zoneIid, zoneName: this.zoneName}})
      },
      goSubzone: function (id, name) {
        router.push({name: 'ReportListSubzone', params: {subzoneId: id, subzoneName: name}})
      }
    },
    components: {
      Icon,
