...
<template>
  <div class="table-wrapper">
    ...
    <table-item v-for="item in items" v-bind:data="item" v-bind:key="item.subzoneId" v-on:click.native="goSubzone(item.subzoneId, item.subzoneName)">
      <a slot="title"><span class="item-left">{{item.subzoneName}}</span></a>
      <a slot="state">
        <a v-for="state in item.states">
          <span class='subzone-class'>{{state.stateName}}</span>
  ...
  export default {
    ...
    methods: {
      fetchData: function () {
        ...
        HTTP.get('/plan/byzone?zone=' + this.zoneId)
...
//            console.log(response.data)
      ...
      goSubzone: function (id, name) {
        router.push({name: 'CheckListSubzone', params: {subzoneId: id, subzoneName: name}})
      }
    },
    components: {
      HTTP,
