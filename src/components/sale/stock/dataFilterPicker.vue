<!--/src/components/sale/retail/dataFilterPicker.vue-->
<template lang="html">
	...
	export default {
		...
		methods: {
			...
			getStatisticTypes() {
				let _this = this
				_this.data = []
                _.forEach(_this.statisticTypes, function(elem) {
					_this.data.push({ value: elem })
				})
			},
			...
			handleStatisticTypeChange(tabIndex) {
                this.statisticType = tabIndex
				this.$emit('transmit', {
					statisticType: {
					   type: tabIndex
					}
			...
			handleParentTabChange(tabIndex, newTab, oldTab) {
				switch(this.filterType) {
					...
					break
					case 2 : this.handleStatisticTypeChange(tabIndex)
					break
					default: 
					break
				}
